...
일반적으로 메서드가 오버라이드 되는 경우에 자신으로부터 가장 가까운 메서드에만 접근할 수 있지만,
그 다음으로 가까운 __proto__의 메서드도 우회적인 방법을 통해 접근도 가능하다.
...
배열의 내부 구조
__proto__내부에 pop push등이 있음
다시또 그 안의 __proto_안에 동일한 애요으로 이 내용이 있음
이는 prototype 객체가 바로 '객체'이기 때문임.

즉
[1,2] <— Array.prototype <— Object.prototype
이렇듯 __proto__ 프로퍼티 내부에 다시 __proto__ 프러퍼티가 연쇄적으로 이어진 것을 플포 타입 체인,
이 체인을 따라가며 검색하는 것을 프로토 타입 체이닝이라고 함.

메서드 오버라이드와 동일한 맥락임.

이렇듯 결국에는 Object.prototype이 맨 위에 있게끔 있다.
각 생성자 함수는 모두 함수이기 때문에 Function 생성자 함수의 prototype과 연결됨.
Function 생성자 함수 역시 함수이므로, 다시 Function 생성자 함수의 prototype과 연결.
이런식으로 재귀적으로 반복하다 보면 끝없이 올라감.

실제로 이런 데이터를 저장하고 있는것은 아니고 브라우저에 만약 요청한다면
그때야 만들기때문에 데이터를 가지고 있지는 않음.
그리고 크게 의미는 없음.

6.2.3 객체 전용 메서드의 예외사항

위에서 말한대로 어떤 생성자 함수든 prototype은 반드시 객체이기 때문에 Object.prototype이 언제나 최상단에 존재함.
